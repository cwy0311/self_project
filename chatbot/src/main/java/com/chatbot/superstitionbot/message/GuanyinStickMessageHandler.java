...
public class GuanyinStickMessageHandler implements MessageHandler{
	private String order=MessageHandlerFactory.guanyin;
	
	@Override
	public SendMessage updateMessage(Message message) {
	      int stickNumber = new Random().nextInt(100)+1;
	      String result=StickPropertyUtil.getGuanyinStickResult(stickNumber);
	      String poem=StickPropertyUtil.getGuanyinStickPoem(stickNumber);   
	...
	public void popupDescription(CallbackQuery callbackQuery,SuperstitionBot bot) {
	    ...
	    String description=StickPropertyUtil.getGuanyinStickDescription(stickNumber);
	    try {
