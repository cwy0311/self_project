...
public class EmperorguanStickMessageHandler implements MessageHandler{
	private String order=MessageHandlerFactory.emperorguan;
	
	@Override
	public SendMessage updateMessage(Message message) {
	      int stickNumber = new Random().nextInt(100)+1;
	      String title=StickPropertyUtil.getEmperorguanStickTitle(stickNumber);
	      String poem=StickPropertyUtil.getEmperorguanStickPoem(stickNumber);   
	...
	public void popupDescription(CallbackQuery callbackQuery,SuperstitionBot bot) {
	    ...
	    String description=StickPropertyUtil.getEmperorguanStickDescription(stickNumber);
	   // String paraPhrase=StickPropertyUtil.getEmperorguanStickParaphrase(stickNumber);
	    try {
